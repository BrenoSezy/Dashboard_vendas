...
# Converter os dados para um arquivo 
@st.cache_data 
def converte_csv(df):
...
def mensagem_sucesso():
    sucesso = st.sucess('Arquivo baixado com sucesso!')
    time.sleep(5)
    sucesso.empty()
...
# Filtragem utilizando query do pandas
# Coluna com nome = `nome` in @nome
# Coluna com variável (numeros) = @nome[0] <= `nome da coluna` <= @nome[1]
query = '''
...
st.markdown(f'A tabela possui :blue[{dados_filtrados.shape[0]}] linhas e :blues[{dados_filtrados.shape[1]} colunas]')
